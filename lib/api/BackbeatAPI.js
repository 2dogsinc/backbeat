...
class BackbeatAPI {
    ...
    _getFailedCRRResponse(cursor, keys, cb) {
        ...
        let [filteredKeys, duplicateKeys] = this._filterFailObjectKeys(keys);
    ...
    getFailedMetrics(details, cb, data) {
        ...
        // test batches of 25
        this._redisClient.scanUniques(pattern, 25, (err, keys) => {
            if (err) {
    ...
    _filterFailObjectKeys(keys) {
        ...
        // TODO: do not normalize for gets
        // attempt to remove without blocking request
        // this._normalizeFailMetrics(duplicateKeys);
        return [filteredKeys, duplicateKeys];
    }
